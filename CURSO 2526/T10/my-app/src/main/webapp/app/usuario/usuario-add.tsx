...
export default function UsuarioAdd() {
  ...
  useDocumentTitle(t('usuario.add.headline'));
  ...
  const createUsuario = async (data: UsuarioDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/usuarios', data);
      navigate('/usuarios', {
            state: {
              msgSuccess: t('usuario.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="d-flex flex-wrap mb-4">
      <h1 className="flex-grow-1">{t('usuario.add.headline')}</h1>
      <div>
        <Link to="/usuarios" className="btn btn-secondary">{t('usuario.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createUsuario)} noValidate>
      ...
      <input type="submit" value={t('usuario.add.headline')} className="btn btn-primary mt-4" />
    </form>
  </>);
}
